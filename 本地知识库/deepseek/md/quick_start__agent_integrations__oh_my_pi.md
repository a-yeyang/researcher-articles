...
### Thinking（思考模式）[​](#thinking思考模式 "Thinking（思考模式）的直接链接")
...
### 三项关键 compat（必配）[​](#三项关键-compat必配 "三项关键 compat（必配）的直接链接")
...
DeepSeek V4 在非官方 provider（DeepInfra、KiloCode、NVIDIA NIM、Zenmux 等）上通过 OpenAI-compatible 接口使用时，reasoning\_content 回传规则可能不同，兼容性未收敛。建议优先用官方 `api.deepseek.com`。
